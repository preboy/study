...
define phashmap
	if $argc==0
		...
		set $bucket = $arg0._M_ht._M_buckets
		set $size = $arg0._M_ht._M_buckets.size()	
		set $i = 0
		while $i < $size
			
			if *($bucket._M_impl._M_start + $i) != 0
				printf "elem[%u]: ", $i
				p (*($bucket._M_impl._M_start + $i))._M_val
			end
			set $i++
		end
		printf "HashMap size = %u\n",$arg0._M_ht._M_num_elements
	end
end

document phashmap
	Prints hashmap information.
	Syntax: phashmap <hashmapName>
	Example:
	phashmap Hashmap - Prints content, size of Hashmap
end 
...
###################################### By Preboy.ZHANG
set prompt \033[0;35m(gdb)\033[0m
