...
class RecentActiveFilesEventListener(sublime_plugin.EventListener):
    def on_pre_close(self, view):
        if view.file_name():
            view.window().run_command("recent_files_closed", { "file_name": view.file_name() })

    def on_modified(self, view):
        if view.file_name():
            view.window().run_command("recent_files_modified", { "file_name": view.file_name() })


class RecentFilesClosedCommand(sublime_plugin.WindowCommand):
...
class RecentFilesModifiedCommand(sublime_plugin.WindowCommand):
    def __init__(self, window):
        sublime_plugin.WindowCommand.__init__(self, window)
        self.recent_active_files = []
